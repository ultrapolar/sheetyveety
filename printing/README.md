...
| `printer_discovery.py` | Lists printers Windows already knows about, for the Settings GUI's "Detect printers on this PC" button. Windows-only (needs `pywin32`); importable anywhere else without error, it just reports detection as unavailable. |
...
| `test_printer_discovery.py` | 6 tests for printer detection, with a fake `win32print` -- runs on any OS. |
...
Printers don't have to be typed in by exact name: "Detect printers on this
PC" (`printer_discovery.py`, needs `pywin32`) lists everything Windows
already has a name for -- local, shared from another PC, or an installed
network/IP printer -- and adds anything new, so bringing a new printer online
is: add it in Windows once, then click Detect here. It can't know a new
printer's duplex/pacing, so that stays a manual step per printer.
...
py test_printer_discovery.py
```

Expect `26/26`, `26/26`, and `6/6 passed`. Run these after any change to file
matching, the decision tree, watermarking, the settings schema, or printer
detection -- that's where the subtle bugs live. None of these three need a
display, and `test_printer_discovery.py` doesn't even need Windows or
pywin32 -- it fakes `win32print`. `settings_gui.py` itself isn't covered by
an automated test (it needs Tkinter and a display), so a change there is
